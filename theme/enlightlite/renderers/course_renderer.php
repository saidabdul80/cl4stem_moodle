...
class theme_enlightlite_core_course_renderer extends core_course_renderer {
        ...
        ?>
            <script>
                window.onload = function(){
                    document.getElementById('frontpage-category-names').classList.add('containter');
                }
            </script>
        <?php
        return $content;
    ...
    public function top_course_menu() {
        ...
        $topmmenu = '<ul class="dropdown-menu forcustom123">'.$mstrcourse.'
    ...
    public function frontpage_available_courses() {
        ...
        $header .= html_writer::start_tag('div',
            array( 'class' => 'available-courses bg-white', 'id' => 'available-courses') );
        ...
        $header .= html_writer::start_tag('div', array('class' => 'container text-dark'));
        $header .= html_writer::tag('h2', get_string('availablecourses'),array('class' => 'text-dark'));
       ...
       /* if ($tcount > '1') {
        ...
        $header .= html_writer::start_tag('div', array( 'class' => " $sliderclass col-md-12 p-0") );
      ...
      /*      foreach ($acourseids as $courseids) {
                ...
                foreach ($acourseids as $courseid) {
                    ...
                    $container .= html_writer::start_tag('div', array( 'class' => 'col-sm-6 col-md-2 ') );
                    $container .= html_writer::start_tag('div', array( 'class' => 'available-content shadow-smx course-containerx',));
                    $container .= html_writer::start_tag('div', array( 'class' => 'available-img imagex'));
                    ...
                    $container .= html_writer::start_tag('div', array( 'class' => 'abt-course-holder'));

                        $container .= html_writer::tag('h6', html_writer::tag('a',
                            $course->get_formatted_name(),array( 'href' => $courseurl, 'class'=>'text-dark text-left pb-0 ' ) ),
                        array('class' => 'title-text text-dark text-left mb-0') );
                        $container .= html_writer::tag('p',html_writer::tag('span','Start: ',array( 'class'=>'text-dark' ) ).' <span class="text-success">'.date('Y-m-d',$course->startdate).'</span>', array('class' => 'title-text text-dark my-0 pb-4 pl-2 text-left') );
                        /* $container .= html_writer::tag('span',substr($course->summary, 0,100).'...',array('class' => 'title-text text-dark my-0 summary-cutx') ); */
                        $container .="  <div class='middlex'>
                                            ".
                                            html_writer::tag('h6',$course->get_formatted_name(),array('class' => 'title-text text-white text-center mb-0 todropx') )
                                            ."
                                            <a href='".$courseurl."' class='textx'>Open</a>
                                        </div>
                      ";
                    $container .= html_writer::end_tag('div');
                    $container .= html_writer::end_tag('div');
    ...
    public function course_search_form($value = '', $format = 'plain') {
        ...
        $output = html_writer::start_tag('div', array('class' => 'search-block p-0', 'style'=>'background:#523f6d;'));
        $output .= html_writer::start_tag('div', array('class' => 'container'));

        if ($this->page->pagelayout == "frontpage") {
            $output .= html_writer::tag('h2', get_string('search_courses', 'theme_enlightlite'), array('class'=>'text-white pt-3'));
        }
        ...
        $output .= html_writer::start_tag('form',
            array(
                'id' => $formid,
                'action' => $searchurl,
                'method' => 'get', 'style'=>'background:transparent '));
